		...
		if ( ! empty( $contents ) && is_array( $contents ) ) {
			...
			if ( isset( $current_screen ) ) {
				...
				foreach ( $contents as $content ) {
					...
					$id      = $this->apply_filters( 'help_tag_id', \Technote\Models\Utility::array_get( $content, 'id', $this->get_help_tab_id( $index ) ), $content, $slug, $index );
					$title   = $this->apply_filters( 'help_tag_title', \Technote\Models\Utility::array_get( $content, 'title', 'Help Tab' ), $content, $slug, $index );
